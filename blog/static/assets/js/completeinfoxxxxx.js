var completeinfo = new Vue({
    ...
    data:{
        ...
        responseMsg: '',
        resume: '',
        resumeName: 'Select a resume to be uploaded',
        bar: ''
    },
    mounted: function(){
    ...
    methods:{
        changeResume: function(){
            completeinfo.photo = this.$refs.resume.files[0]
            // console.log(completeinfo.photo)
            completeinfo.resumeName = this.$refs.resume.files[0].name
        //    console.log(this.$refs.resume.files[0].name)
        },
        ...
        submit: function(){
            ...
            if(completeinfo.expertise == ""){
            ...
            }else if(completeinfo.newSkills.concat(completeinfo.addedSkills).length == 0){
                ...
                data.append("resume", completeinfo.photo)
                for(var i = 0; i < completeinfo.addedSkills.length; i++){
                    addedSkillsClean.push(completeinfo.addedSkills[i].value)
                }
               
                data.append("skills", completeinfo.newSkills.concat(addedSkillsClean))
                axios.post("/setmoreinfo", data, {
                    headers: {'X-CSRFToken': $('input[name=csrfmiddlewaretoken]').val()}, 
                    progress:( progressEvent ) => {
                        completeinfo.bar = Math.round( (progressEvent.loaded * 100) / progressEvent.total )
                    }
            }).then(function(response){
                    if(response.data === "Success"){
                        window.location.href="/verify"
                    }else{
                        completeinfo.responseMsg = response.data
                        $(".Modal").show()
                    }
