...
var settingsOtherInfo = new Vue({
    ...
    methods:{
        selectResume(){
            settingsOtherInfo.resume = this.$refs.resumeFile.files[0]
            settingsOtherInfo.filename = this.$refs.resumeFile.files[0].name
           
        },
        updateResume: function(){
            let data = new FormData()
            data.append("resume", settingsOtherInfo.resume)
            axios.post("/user/updateResume", data, {headers: {'X-CSRFToken': $('input[name=csrfmiddlewaretoken]').val()}}).then(function(response){
               
                if(response.data == "Success"){
                  
                    changeDp.responseMsg = "Resume updated successfully"
                }else{
                    changeDp.modalLogo = "err"
                    changeDp.responseMsg = "Unable to update resume, Please try again!"
                }
                $("#promptModal").show()
            }).catch(function(err){
                console.log(err)
        ...
        updateInfo: function(){
            ...
            if(this.arrIndex != ''){
                ...
                data.append("countryAbbr", this.countryFlagImg.slice(0,this.countryFlagImg.length - 4))
                data.append("country", this.myCountry)
            }
