...
var questions = new Vue({
    ...
    methods:{
        ...
        viewQuestion: function(id, questionSelected){
            var slugifiedQuestion = ''
            var expSpace = new RegExp(' ', 'g')
            var patternSpace = expSpace
            slugifiedQuestion = questionSelected.replace(patternSpace, '-')
           
            window.location.href= questions.url+slugifiedQuestion.replace('?','').toLowerCase()+"/"+id
        },
                ...
                if (this.newTag != "") {
                    if(this.tags.length != 0){
                        ...
                        for(var i = 0; i < this.tags.length; i++){
                            tempTags.push(this.tags[i].toLowerCase())
                        }
                        if(tempTags.indexOf(this.newTag) < 0){
